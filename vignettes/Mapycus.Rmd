...
`sf_fisheye()` chooses a sensible projected working CRS and normalizes coordinates around a center. With `preserve_aspect = TRUE` (default), radii are interpreted in unit-like space (≈ fraction of the layer's half-span).
...
vic_cbd <- sf_fisheye(
  ...
  zoom_factor = 1.5, squeeze_factor = 0.30
)
...
Guidelines for common choices (with `preserve_aspect = TRUE`):

- Radii: `r_in` ≈ 0.25-0.35, `r_out` ≈ 0.55-0.70
- Magnification: `zoom_factor` 1.5-2.0
- Glue compression: `squeeze_factor` 0.25-0.40
- Method: `"expand"` (default) or `"outward"` (stabilizes the outer boundary)
- Revolution: 0 for publication; ≤ 0.2 for demonstrations

```{r parameter-sweep, fig.height=3.5}
params <- expand.grid(
  r_in = c(0.30, 0.34), r_out = c(0.60, 0.70),
  zoom_factor = c(1.4, 1.8), squeeze_factor = c(0.30, 0.40),
  KEEP.OUT.ATTRS = FALSE
)

examples <- lapply(seq_len(nrow(params)), function(i) {
  p <- params[i, ]
  out <- sf_fisheye(vic, center = melbourne,
                    r_in = p$r_in, r_out = p$r_out,
                    zoom_factor = p$zoom_factor,
                    squeeze_factor = p$squeeze_factor)
  out$label <- sprintf("r_in=%.2f r_out=%.2f\nzoom=%.1f squeeze=%.2f",
                       p$r_in, p$r_out, p$zoom_factor, p$squeeze_factor)
  out
})

examples <- do.call(rbind, examples)

ggplot(examples) +
  geom_sf(fill = "grey92", color = "white", linewidth = 0.15) +
  facet_wrap(~ label, ncol = 2) +
  ggtitle("Parameter effects: radii, magnification, glue compression")
```
...
both_fish <- sf_fisheye(both, center = melbourne,
                        r_in = 0.34, r_out = 0.60,
                        zoom_factor = 1.6, squeeze_factor = 0.35)
